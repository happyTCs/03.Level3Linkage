...
public class ScrollerNumberPicker extends View {
	...
	/*	*//**
		...
		public void drawSelf(Canvas canvas) {
			 ...
			 * 操作人： 高晨
			 * 时间：   2019-10-11
			 * 原因：   原算法导致文字字体过大，字符显示不全
			 * 做法：   当控件宽度容纳不下全部字符，以原字体大小的二分之一重新计算字体所占宽度
			 */
			/******************************* GCN--修改--开始 **********************************************/
			if(controlWidth - textRect.width() < 60)
			{
				// 判断是否被选择
				if (isSelected()) {
					textPaint.setColor(selectedColor);
			 ...
			 * 操作人： 高晨
			 * 时间：   2019-10-11
			 * 原因：   原算法文字画笔起点位于屏幕外，导致文字显示不全
			 */
			// 绘制内容
			/******************************* GCN--修改--开始 **********************************************/

			float startX = x + controlWidth / 2 - textRect.width() / 2;
			canvas.drawText(itemText, startX < 0 ? 10 :startX,
					y + move + unitHeight / 2 + textRect.height() / 2,
					textPaint);
			/******************************* GCN--修改--结束 **********************************************/
		}
		/**
